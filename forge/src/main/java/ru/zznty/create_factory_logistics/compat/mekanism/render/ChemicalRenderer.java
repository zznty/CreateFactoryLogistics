...
public class ChemicalRenderer {
    public static void renderChemicalBox(Holder<Chemical> chemical, PoseStack ms, MultiBufferSource buffer, int light,
                                         float xMin, float yMin, float zMin, float xMax, float yMax,
                                         float zMax) {
        if (chemical.is(MekanismAPI.EMPTY_CHEMICAL_HOLDER)) return;

        // mekanism does not use alpha channel
        int color = chemical.value().getTint() | 0xff_000000;
