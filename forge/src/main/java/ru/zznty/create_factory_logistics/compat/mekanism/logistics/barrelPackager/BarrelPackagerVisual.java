...
public class BarrelPackagerVisual<T extends BarrelPackagerBlockEntity> extends AbstractBlockEntityVisual<T> implements SimpleDynamicVisual {
    public final TransformedInstance tray;
    public float lastTrayOffset = Float.NaN;

    public BarrelPackagerVisual(VisualizationContext ctx, T blockEntity, float partialTick) {
        super(ctx, blockEntity, partialTick);

        tray = instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(
                        BarrelPackagerRenderer.getTrayModel(blockState)))
                .createInstance();
