...
public class FactoryChemicalPanelBehaviour extends AbstractFactoryPanelBehaviour implements GenericFilterProvider, PanelModelProvider {
    ...
    public Holder<Chemical> getChemicalHolder() {
        ...
        return getChemicalStack(filter.item()).getChemicalHolder();
    }
    ...
    public static ChemicalStack getChemicalStack(ItemStack stack) {
        ...
        if (ChemicalUtil.hasAnyChemical(stack))
        {
            IChemicalHandler cap = stack.getCapability(Capabilities.CHEMICAL.item());
            if (cap != null)
                return getChemical(cap);
        }
    ...
    private static ChemicalStack getChemical(IChemicalHandler handler) {
        if (handler.getChemicalTanks() < 1) return ChemicalStack.EMPTY;
    ...
    public GenericStack filter() {
        ...
        return ChemicalGenericStack.wrap(new ChemicalStack(getChemicalHolder(), count));
    }
