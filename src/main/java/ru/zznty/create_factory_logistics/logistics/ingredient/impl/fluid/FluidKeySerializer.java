...
public class FluidKeySerializer implements IngredientKeySerializer<FluidIngredientKey> {
    ...
    public <T> Codec<IngredientKey<T>> codec() {
        Codec codec = FluidStack.CODEC.xmap(
                stack -> new FluidIngredientKey(stack.getFluidHolder(), stack.getComponents()),
                FluidIngredientKey::stack);
        return codec;
    }
    ...
    public void write(HolderLookup.Provider levelRegistryAccess, FluidIngredientKey key, CompoundTag tag) {
        ...
        if (key.components() != null)
            tag.put("Tag", DataComponentPatch.CODEC.encode(key.components().asPatch(),
                                                           levelRegistryAccess.createSerializationContext(
                                                                   NbtOps.INSTANCE),
    ...
    public FluidIngredientKey read(HolderLookup.Provider levelRegistryAccess, CompoundTag tag) {
        ...
        return key.isEmpty() ? new FluidIngredientKey(FluidStack.EMPTY.getFluidHolder(), null) :
               new FluidIngredientKey(levelRegistryAccess.holderOrThrow(BuiltInRegistries.FLUID.getResourceKey(
                       BuiltInRegistries.FLUID.get(ResourceLocation.parse(key))).get()),
                                      tag.contains("Tag") ?
                                      PatchedDataComponentMap.fromPatch(DataComponentMap.EMPTY,
                                                                        DataComponentPatch.CODEC.decode(NbtOps.INSTANCE,
                                                                                                        tag.getCompound(
    ...
    public FluidIngredientKey read(RegistryFriendlyByteBuf buf) {
        return new FluidIngredientKey(buf.registryAccess()
                                              .holderOrThrow(BuiltInRegistries.FLUID.getResourceKey(
                                                      BuiltInRegistries.FLUID.get(buf.readResourceKey(
                                                              BuiltInRegistries.FLUID.key()))).get()),
                                      PatchedDataComponentMap.fromPatch(DataComponentMap.EMPTY,
